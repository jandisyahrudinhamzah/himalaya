  ...
  <div class="rounded-2xl px-6 py-4 flex justify-between items-center bg-slate-900/30 backdrop-blur-lg border border-white/20 shadow-lg" style="position:relative">
    <div class="flex items-center gap-3 cursor-pointer group">
      <div class="relative group">
        <img src="{{ asset('image/himalaya1.jpeg') }}" alt="Logo"
    ...
    <div class="hidden md:flex items-center gap-1">
      <a href="{{ route('public.home') }}"       class="px-5 py-2.5 rounded-full text-sm font-medium text-slate-300 hover:text-orange-400 hover:bg-slate-800/50 transition-all">Beranda</a>
      <a href="{{ route('public.articles') }}"   class="px-5 py-2.5 rounded-full text-sm font-medium text-slate-300 hover:text-orange-400 hover:bg-slate-800/50 transition-all">Artikel</a>
      <a href="{{ route('public.gallery') }}"    class="px-5 py-2.5 rounded-full text-sm font-medium text-slate-300 hover:text-orange-400 hover:bg-slate-800/50 transition-all">Galeri</a>
      <a href="{{ route('public.activities') }}" class="px-5 py-2.5 rounded-full text-sm font-medium text-slate-300 hover:text-orange-400 hover:bg-slate-800/50 transition-all">Kegiatan</a>
      <a href="{{ route('public.members') }}"    class="px-5 py-2.5 rounded-full text-sm font-medium text-slate-300 hover:text-orange-400 hover:bg-slate-800/50 transition-all">Anggota</a>
    </div>
    <div class="flex items-center gap-4">
      <a href="{{ route('login') }}" class="hidden md:flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-orange-600 to-orange-500 text-white text-sm font-semibold rounded-full hover:shadow-lg hover:shadow-orange-500/30 transition-all transform hover:-translate-y-0.5">
    ...
    <div class="mob-menu md:hidden" id="mobileMenu">
      ...
      <a href="{{ route('public.gallery') }}">Galeri</a>
      <a href="{{ route('public.activities') }}">Kegiatan</a>
      <a href="{{ route('public.members') }}">Anggota</a>
      <a href="{{ route('login') }}" style="background:rgba(249,115,22,0.14);color:#f97316;margin-top:4px">Masuk</a>
    </div>
  </div>
</nav>

{{-- ════ HERO (UNCHANGED) ════ --}}
<div class="bg-cover bg-center bg-fixed min-h-screen" style="background-image:url('{{ asset('image/petacompas.jpg') }}')">
...
<div class="ticker-shell">
  ...
  <div style="padding-left:100px;overflow:hidden;width:100%">
    <div class="ticker-track" id="ticker">
      @php $artikels=\App\Models\Artikel::where('status','published')->latest()->take(7)->get(); @endphp
      @foreach($artikels as $a)
        <a href="{{ route('public.article.show',$a->id) }}" class="t-item">
          @if($a->gambar)<img src="{{ asset('storage/'.$a->gambar) }}" alt="">@endif
          <span class="t-dot"></span><span>{{ $a->judul }}</span>
        </a>
      @endforeach
      @foreach($artikels as $a)
        <a href="{{ route('public.article.show',$a->id) }}" class="t-item">
...
<section class="sec sec-a">
  <div class="wrap">
    ...
    <div class="bento">
      @php $colors=['#f97316','#ea580c','#fb923c','#fbbf24','#f59e0b','#ef4444','#22c55e','#6366f1','#a855f7','#06b6d4']; @endphp
      @foreach($struktur as $i => $item)
        @php $c=$colors[$i%count($colors)]; @endphp
        <div class="sk-card fu" style="transition-delay:{{ $i*0.055 }}s">
          @if($item->foto)
            <img src="{{ asset('storage/'.$item->foto) }}" alt="{{ $item->nama }}" class="sk-av">
          @else
            @php $ini=collect(explode(' ',$item->nama))->take(2)->map(fn($w)=>strtoupper($w[0]))->join(''); @endphp
            <div class="sk-init" style="background:{{ $c }}16;border-color:{{ $c }}38;color:{{ $c }}">{{ $ini }}</div>
          @endif
          <div class="sk-role">{{ $item->jabatan }}</div>
          <div class="sk-name">{{ $item->nama }}</div>
  ...
  <div class="wrap">
    <div class="about-split">
      ...
      <div class="fu" style="transition-delay:.17s">
        ...
        <div class="stats-row">
          <div class="stat-tile"><div class="stat-n">{{ $stats['total_anggota'] }}</div><div class="stat-l">Anggota</div></div>
...
<section class="sec sec-b">
  <div class="wrap">
    <div class="keg-flex fu">
      ...
      <a href="{{ route('public.activities') }}" class="see-all">Lihat Semua <i class="fas fa-arrow-right"></i></a>
    </div>
    <div class="keg-grid">
      @forelse(\App\Models\Kegiatan::latest()->take(3)->get() as $i => $kegiatan)
        <div class="keg-card fu" style="transition-delay:{{ $i*0.08 }}s">
          <div class="keg-thumb">
            @if($kegiatan->foto)<img src="{{ asset('storage/'.$kegiatan->foto) }}" alt="{{ $kegiatan->judul }}">
            @else<div class="keg-ph"><i class="fas fa-mountain" style="font-size:38px;color:var(--tx3)"></i></div>@endif
            <div class="keg-overlay"></div>
            <div class="keg-meta">
              <div class="keg-date"><i class="far fa-calendar-alt"></i> {{ \Carbon\Carbon::parse($kegiatan->tanggal)->format('d M Y') }}</div>
              <span class="keg-status {{ $kegiatan->status=='Upcoming'?'s-up':($kegiatan->status=='Ongoing'?'s-on':'s-dn') }}">{{ $kegiatan->status }}</span>
            </div>
          </div>
          <div class="keg-body">
            <h3 class="keg-h">{{ $kegiatan->judul }}</h3>
            <p class="keg-p">{{ Str::limit($kegiatan->deskripsi,90) }}</p>
            <a href="{{ route('public.activity.show',$kegiatan->id) }}" class="keg-btn">Detail Kegiatan <i class="fas fa-arrow-right"></i></a>
          </div>
        </div>
      @empty
...
<section class="sec sec-a" style="padding:80px 0">
  <div class="wrap">
    <div class="cta-banner fu">
      ...
      <div class="btn-row">
        <a href="{{ route('public.gallery') }}" class="btn-fire"><i class="fas fa-folder-open"></i> Lihat Galeri</a>
...
<section class="sec sec-b">
  <div class="wrap">
    <div class="fu" style="display:flex;align-items:flex-end;justify-content:space-between;flex-wrap:wrap;gap:16px">
      ...
      <a href="{{ route('public.articles') }}" class="see-all">Lihat Semua <i class="fas fa-arrow-right"></i></a>
    </div>
    <div class="art-grid">
      @forelse(\App\Models\Artikel::where('status','published')->latest()->take(3)->get() as $i => $artikel)
        <a href="{{ route('public.article.show',$artikel->id) }}" class="art-card fu" style="transition-delay:{{ $i*0.08 }}s">
          <div class="art-thumb">
            @if($artikel->gambar)<img src="{{ asset('storage/'.$artikel->gambar) }}" alt="{{ $artikel->judul }}">
            @else<div class="art-ph"><i class="fas fa-newspaper" style="font-size:32px;color:var(--tx3)"></i></div>@endif
          </div>
          <div class="art-body">
            <span class="art-pill">Artikel</span>
            <h3 class="art-h">{{ $artikel->judul }}</h3>
            <div class="art-foot"><i class="far fa-calendar"></i> {{ \Carbon\Carbon::parse($artikel->created_at)->format('d M Y') }}</div>
          </div>
        </a>
      @empty
  ...
  <div class="wrap">
    <div class="ft-main">
      ...
      <div class="fu" style="transition-delay:.07s">
        ...
        <ul class="ft-links">
          <li><a href="{{ route('public.home') }}"><span class="ldot"></span>Beranda</a></li>
          <li><a href="{{ route('public.articles') }}"><span class="ldot"></span>Artikel</a></li>
          <li><a href="{{ route('public.gallery') }}"><span class="ldot"></span>Galeri</a></li>
          <li><a href="{{ route('public.activities') }}"><span class="ldot"></span>Kegiatan</a></li>
          <li><a href="{{ route('public.members') }}"><span class="ldot"></span>Anggota</a></li>
        </ul>
      </div>
      ...
      <div class="fu" style="transition-delay:.12s">
        ...
        <ul class="ft-links">
          ...
          <li><a href="{{ route('login') }}"><span class="ldot"></span>Login Admin</a></li>
        </ul>
      </div>
    ...
    <div class="ft-bottom fu" style="transition-delay:.23s">
      <p class="ft-copy">&copy; {{ date('Y') }} <span>HIMAYALA</span>. All rights reserved.</p>
